...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    HRESULT(WINAPI * fnD3DCompile)(_In_reads_bytes_(SrcDataSize) LPCVOID pSrcData,
        ...
        _Always_(_Outptr_opt_result_maybenull_) ID3DBlob * *ppErrorMsgs) = nullptr;

    HMODULE CompilerDll = 0;
    for (int i = 0; i < 100; i++)
    {
        CompilerDll = LoadLibraryA(("D3DCompiler_" + std::to_string(i) + ".dll").c_str());
        if (CompilerDll)
        {
            FARPROC fnAddress = GetProcAddress(CompilerDll, "D3DCompile");
            if (fnAddress)
            {
                fnD3DCompile = reinterpret_cast<decltype(fnD3DCompile)>(fnAddress);
                break;
            }
        }
    }
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(fnD3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL)))
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(fnD3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
