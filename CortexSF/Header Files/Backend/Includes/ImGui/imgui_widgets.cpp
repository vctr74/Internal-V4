...
bool ImGui::ButtonExx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags )
{
  ...
  //  window->DrawList->AddRect(rect.Min, rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);
    ...
    ImGui::PushStyleColor( ImGuiCol_Text, ImVec4( ImColor( 255, 255, 255, 100 + it_anim->second.active_rect_alpha ) ) );
    ImGui::RenderTextClipped( rect.Min + ImVec2( 0, 1 ), rect.Max + ImVec2( 0, 1 ), label, NULL, &label_size, ImVec2( 0.5f, 0.5f ) );
    ImGui::PopStyleColor( );
...
bool ImGui::ButtonNew( const char* label, const ImVec2& size_arg )
{
    return ImGui::ButtonExx( label, size_arg, ImGuiButtonFlags_None );
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, const char* txt)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
